...
public class MongoCodec {
	...
	private Object retrieveFile(String name) throws Exception {
		...
		try {
			...
			String calculated = Hex
					.toHexString(digester.digest().toByteArray());
	 ...
	 * null) { bdo.put(f.getName(), toBSON(o)); } } }
	 * 
	 * return bdo;
	 * 
	 * @SuppressWarnings({"rawtypes", "unchecked"}) Object toBSON(Object o)
	 * throws Exception { if (o instanceof DBObject || o instanceof DBRef || o
	 * instanceof DBCollection) return o;
	 * 
	 * if (o instanceof File) return storeFile((File) o);
	 * 
	 * if (o == null || o instanceof Number || o instanceof Boolean || o
	 * instanceof Date || o instanceof String || o instanceof ObjectId) return
	 * o;
	 * 
	 * if (o.getClass().isArray()) { Class< ? > cctype =
	 * o.getClass().getComponentType(); if (cctype.isPrimitive()) { if (cctype
	 * != char.class) return o;
	 * 
	 * return new String((char[]) o); }
	 * 
	 * BasicDBList l = new BasicDBList(); for (Object member : (Object[]) o)
	 * l.add(toBSON(member));
	 * 
	 * return l; }
	 * 
	 * if (o instanceof Iterable) { BasicDBList l = new BasicDBList(); for
	 * (Object member : (Iterable) o) l.add(toBSON(member));
	 * 
	 * return l; }
	 * 
	 * if (o instanceof Map) { BasicDBObject bdo = new BasicDBObject(); Map map
	 * = (Map) o; for (Map.Entry entry : (Set<Map.Entry>) map.entrySet()) {
	 * bdo.put(entry.getKey().toString(), toBSON(entry.getValue())); } return
	 * bdo; }
	 * 
	 * if ( data.isData(o)) return to(o);
	 * 
	 * return o.toString(); }
	 * 
	 * 
	 * T instance = type.newInstance(); for (Field f : fields) { Object object;
	 * 
	 * if (f.getType() == File.class) object = retrieveFile((String)
	 * dbo.get(f.getName())); else object =
	 * converter.convert(f.getGenericType(), dbo.get(f.getName())); if (object
	 * != null) f.set(instance, object); }
	 * 
	 * return instance; private Object fromBson(String key, String value) throws
	 * Exception { Object o; try { Field f = type.getField(key); if (f.getType()
	 * == File.class) return retrieveFile(key);
	 * 
	 * o = converter.convert(f.getGenericType(), value); } catch (Exception e) {
	 * o = converter.convert(Object.class, value); }
	 * 
	 * // Enums are not properly handled by mongo if (o instanceof Enum) o =
	 * value;
	 * 
