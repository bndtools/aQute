...
public class SerializabilityUtil {
  ...
  /**
   * Provide a replacement for {@link Class#forName(String, boolean, ClassLoader)} that does NOT pin classes
   * in memory but always use the class loader. However, the class
   * loader does not handle arrays nor primitives so this method
   * acts the same as what you can give Class.forName
   */
  public static Class<?> forName( String name, ClassLoader classLoader ) throws ClassNotFoundException {
	  // Handle arrays, done recursively
	if (name.startsWith("[")) {
		Class<?> componentType = forName(
				name.substring(1),
				classLoader);
		return Array.newInstance(componentType, 0).getClass();
	}
	// primitives are one character classes.
	// obviously you can fool this but no sane persion 
	// creates a one letter class in the default package?
	if (name.length() == 1 ) {
		...
		// And leave it up to the given class loader
