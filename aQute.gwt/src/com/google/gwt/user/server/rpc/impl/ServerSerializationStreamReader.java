...
public final class ServerSerializationStreamReader extends
  ...
  /*
   * The class loader is no longer final so that the user of the
   * stream can change it.
   */
    ...
    try {
      ...
      if (hasFlags(FLAG_ELIDE_TYPE_NAMES)) {
        if (getSerializationPolicy() instanceof TypeNameObfuscator) {
          ...
          // This used to be Class.forName but this method pins
          // methods in memory and is thus not useable with OSGi.
          instanceClass = SerializabilityUtil.forName(instanceClassName,classLoader);
        } else {
          throw new SerializationException(
        ...
        instanceClass = SerializabilityUtil.forName(serializedInstRef.getName(),
            classLoader);
