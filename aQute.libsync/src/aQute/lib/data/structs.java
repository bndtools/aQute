...
public class structs {
	...
	public static List<Mismatch> validate(Object instance) throws Exception {
		...
		for (Field f : instance.getClass().getFields()) {
			...
			if (match != null)
				reason = validate(match, value);
			else if (cmatch != null)
				reason = validate(cmatch, value);

			if (reason != null)
				mismatches.add(new Mismatch(f, value, reason));
		}
	...
	private static String validate(CollectionMatch match, Object value)
		...
		if (value instanceof Collection) {
			...
			for (Object member : coll) {
				...
				if (s != null) {
					return "Collection member element mismatch, " + s;
				}
			}
		}
		if (value.getClass().isArray()) {
			...
			for (int i = 0; i < length; i++) {
				...
				String s = validate(match.value(), member);
				if (s != null) {
					return "Array member element mismatch, " + s;
				}
			}
