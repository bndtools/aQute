...
public class JSONEncoder {
	public class PeekReader {
		...
		void expect(String s) throws IOException {
			for (int i = 0; i < s.length(); i++)
				check(s.charAt(i) == get(), "Expected " + s
						+ " but got something different");
		}
	}

	public void encode(Appendable app, Type type, Object object)
		...
		check(rawClass.isAssignableFrom(actualClass)
				...
				"Type does not match the object's class " + rawClass + " <= "
						+ object.getClass());
		...
		// We sort the methods so they are canonical
		...
		for (Field field : fields) {
			app.append(del);
			string(app, field.getName());
			app.append(":");
			encode(app, field.getGenericType(), field.get(object));
			del = ",";
		}
		app.append("}");
	}
	...
	private void encodeArray(Appendable app, Type type, Object object)
		...
		for (int i = 0; i < l; i++) {
			app.append(del);
			encode(app, componentType, Array.get(object, i));
			del = ", ";
		}
		app.append("]");
	...
	private void encodeMap(Appendable app, Type type, Map< ? , ? > map)
		...
		for (Map.Entry< ? , ? > entry : map.entrySet()) {
			...
			encode(app, keyType, o);
			app.append(":");
			encode(app, valueType, entry.getValue());
			del = ", ";
		}
		app.append("}");
	}
	...
	public <T> T decode(Class<T> type, InputStream in) throws Exception {
		return type.cast(decode((Type) type, in));
	}

	public Object decode(Type type, InputStream in) throws Exception {
	...
	private Object decode(Type type, PeekReader isr) throws Exception {
		...
		switch (c) {
			case '{' : {
				Class< ? > rawClass = toClass(type);
				if (Map.class.isAssignableFrom(rawClass))
					return doMap(rawClass, type, isr);
				else
					if (String.class == rawClass)
						return doString(isr, '}');
					else
						return doObject(rawClass, isr);
			}
			...
			case '[' : {
				...
				if (type instanceof ParameterizedType) {
					...
					return doCollection(rawClass,
							pt.getActualTypeArguments()[0], isr);
				}
				else
					if (type instanceof GenericArrayType) {
						GenericArrayType gat = (GenericArrayType) type;
						return doArrayType(gat.getGenericComponentType(), isr);
					}
					else
						if (rawClass.isArray())
							return doArrayType(rawClass.getComponentType(), isr);
						else
							if (rawClass == String.class)
								return doString(isr, ']');
							else
								throw new IllegalArgumentException(
										"Got an array [] but result type is not a collection or array: "
												+ rawClass);
			}
	...
	private Object doArrayType(Type type, PeekReader isr) throws Exception {
		Class< ? > rawClass = toClass(type);
		int c = isr.peek();
		List<Object> result = new ArrayList<Object>();
		while (c == '"' || Character.isDigit(c) || c == '-' || c == '['
				|| c == '{') {
	...
	private Object doCollection(Class< ? > rt, Type type, PeekReader isr)
			throws Exception {
		if (rt.isInterface()) {
			if (rt == Collection.class || rt == List.class)
				rt = ArrayList.class;
			else
				if (rt == Set.class || rt == SortedSet.class)
					rt = TreeSet.class;
				else
					if (rt == Queue.class /* || resultType == Deque.class */)
						rt = LinkedList.class;
					else
						if (rt == Queue.class /* || resultType == Deque.class */)
							rt = LinkedList.class;
	...
	private Object doMap(Class< ? > clazz, Type type, PeekReader isr)
			throws Exception {
		...
		while (c == '"') {
			...
			if (keyType != String.class) {
				...
				key = decode(
						keyType,
						new ByteArrayInputStream(((String) key)
								.getBytes("UTF-8")));
			}
	...
	private Object doObject(Class< ? > clazz, PeekReader isr) throws Exception {
			...
			if (c == '\\') {
				...
				switch (c) {
					...
					case 't' :
						...
						break;
					case 'u' :
						c = hexDigit(r.get()) << 12 + hexDigit(r.get() << 8)
								+ hexDigit(r.get() << 4) + hexDigit(r.get());
						break;
