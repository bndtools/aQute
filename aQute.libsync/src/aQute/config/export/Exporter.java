...
public class Exporter {
	...
	public static void toJSON(ConfigurationAdmin admin, Writer osw, String filter)
			throws Exception {
				...
				for (Enumeration<String> e = d.keys(); e.hasMoreElements();) {
					...
					if (!(v instanceof String)) {
						...
						if (v instanceof Collection) {
							...
							clazz = String.class;
							if (coll.size() > 0) type.vectorOf = shortName(coll
	...
	private static String shortName(Class clazz) {
		...
		if (name.startsWith("java.lang.")) return name.substring(10);
		else return name;
	}

	public static void fromJSON(Reader r, ConfigurationAdmin admin)
			throws Exception {
	...
	private static Dictionary<String, Object> getDictionary(
			Map<String, Type> types, Map<String, Object> values)
			throws Exception {
				...
				if (type.scalar != null) {
					...
					if (value instanceof String && type.arrayOf.equals("byte")) {
						...
						Object array = Array.newInstance(
								getClass(type.arrayOf), coll.size());
						int n = 0;
						for (Object o : coll) {
	...
	private static Object convert(Object o, String type) {
		if ("boolean".equals(type)) return o;
		if ("Boolean".equals(type)) return o;

		if ("byte".equals(type)) return ((Number) o).byteValue();

		if ("short".equals(type)) return ((Number) o).shortValue();

		if ("char".equals(type)) return (char) ((Number) o).intValue();

		if ("int".equals(type)) return ((Number) o).intValue();

		if ("long".equals(type)) return ((Number) o).longValue();

		if ("float".equals(type)) return ((Number) o).floatValue();

		if ("double".equals(type)) return ((Number) o).doubleValue();

		if ("String".equals(type)) return o;

		if ("Byte".equals(type)) return ((Number) o).byteValue();

		if ("Short".equals(type)) return ((Number) o).shortValue();

		if ("Character".equals(type)) return (char) ((Number) o).intValue();

		if ("Integer".equals(type)) return ((Number) o).intValue();
		...
		throw new IllegalArgumentException(
				"Invalid class name for Configuration Admin data type " + type);
	}
