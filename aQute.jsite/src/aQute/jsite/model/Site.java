...
public class Site {
	...
	class XmlDoc extends Doc {
		...
		public void transfer(File target) throws Exception {
			...
			defaultTemplate.transform(new StreamSource(new StringReader(xml)),
					result);

			cleanup(result.getNode());
			unity.transform(new DOMSource(result.getNode()), new StreamResult(
					target));
		}
		...
		private void cleanup(Node node) throws XPathExpressionException {
			...
			// marked active.
			NodeList list = (NodeList) xpath.evaluate("//a[@href='" + path
		...
		public void transfer(File target) throws Exception {
			...
			try {
				...
				try {
					...
					writer.flush();
					if (result != 0) {
						reporter.error("executing command failed %s %s",
								command, IO.collect(target) + "\n" + errors);
						target.delete();
					}
				} finally {
					writer.close();
				...
				reporter.error("executing command failed %s %s", command,
						e.getMessage());
			}
		}
	}
	...
	class MarkdownDoc extends XmlDoc {
		...
		String getXmlContent() {
			try {
				return "<content>" + Processor.process(source) + "</content>";
			} catch (IOException e) {
				throw new RuntimeException(e);
			}
		}
	...
	public boolean prepare() throws IOException {
		...
		return rootLevel.lastModified > lastModified
